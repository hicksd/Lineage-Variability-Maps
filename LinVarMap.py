...
"""Find position of irrep boundaries"""
#pos = np.array([0]+[len(list(v)) for k,v in 
#    itertools.groupby(irreps)]).cumsum()-0.5 
"""Plot irrep boundaries"""
#for i in range(len(pos)):
#    lo, hi = pos[i-1], pos[i]
#    ax.plot([lo,hi],[lo,lo],linestyle='dashed',color='k')#upper
#    ax.plot([lo,hi],[hi,hi],linestyle='dashed',color='k')#lower
#    ax.plot([lo,lo],[lo,hi],linestyle='dashed',color='k')#left
#    ax.plot([hi,hi],[lo,hi],linestyle='dashed',color='k')#right

##grps =[np.ones(len(list(v))) for k,v in itertools.groupby(irreps)]
##blk_diag = linalg.block_diag(*[np.outer(v,v) for v in grps])
##ax.contour(dummy,dummy,blk_diag,levels=[0.5])
                
      
"""
        #Create gmax Haar transform matrices and store in dict.
        #Make change-of-basis matrix. Each row is a symmetry-adapted basis 
        #vector. CHECK IF scipy.linalg.block_diag helps
        
        Nvars = 2**gmax-1
        H = {1:np.ones(1),2:np.array([[1.,1.],[1.,-1.]])}
        F = np.zeros((Nvars,Nvars))
        for i in range(2,gmax):
            Ni = H[i].shape[0]
            a  = np.kron(H[i], np.array([1,1]))
            b  = np.kron(np.eye(Ni),np.array([1,-1]))
            H[i+1] = np.concatenate([a,b],axis=0)
        
        
        i=0 # counter for F index
        hctr = {h:0 for h in range(1,gmax+1)} #counters for each Haar matrix
        func = lambda x: (2**(x-1)-1,2**x-2) #start/end indices in F for gen
        modes = [] #store the name of each mode
        for irrep in range(1,gmax+1):
            dim = 1 if irrep==1 else 2**(irrep-2) #dimensionality of irrep
            for d in range(1,dim+1):
                for g in range(irrep,gmax+1):
                    jmin, jmax = func(g)
                    Vg = H[g][hctr[g]] #unnormalized subvector from Haar matrix
                    F[i,jmin:jmax+1] = Vg/np.sqrt(np.dot(Vg,Vg))
                    hctr[g] += 1 # increment the appropriate counter
                    modes.append((g,irrep,d))
                    #modes.append('('+str(irrep)+','+str(g)+','+str(d+1)+')')
                    i+=1
"""
"""
    #ORIGINAL VERSION OF E-step
    def _eStep(self, X, params):
        ...
        observedList = [np.array(np.where(~np.isnan(row))).flatten() 
            for row in X] #each element is an array of indices for that row
        nExamples, dataDim = X.shape
        ...
        for n in range(nExamples):
            ...
            muObs = mu[xo]
            muMiss = mu[xm]
            covObs = cov[np.ix_(xo,xo)]
            covMiss = cov[np.ix_(xm,xm)]
            covObsMiss = cov[np.ix_(xo,xm)]
            covMissObs = cov[np.ix_(xm,xo)]
            row = X[n,:]
            rowObserved = row[xo]
            ...
            # Otherwise deal with missing data
                ...
                meanCond = (muMiss + covMissObs.dot(
                            np.linalg.inv(covObs)).dot(rowObserved - muObs))
                covCond = covMiss - \
                    covMissObs.dot(np.linalg.inv(covObs)).dot(covObsMiss)
                ...
                xxOuter[np.ix_(xm, xm)] = np.outer(meanCond, meanCond) + \
                    covCond
                xxOuterCum = xxOuterCum + xxOuter

                # Non constant terms of entropy of p(x_miss| x_obs, params) 
                # for computation of log likelihood
                cumEnt = cumEnt + 0.5*np.log(np.linalg.det(covCond))
        ...
        # Expected complete data log-likelihood
        ell = (
             - 0.5*nExamples*np.log(np.linalg.det(2*np.pi*cov))
             - 0.5*np.trace(np.linalg.inv(cov).dot(
               xxOuterCum + nExamples*np.outer(mu, mu) 
               - 2*np.outer(mu, xTotCum)))
              )
...
"""Graphplot """
        ...
        #    x = int(np.log2(i))
        #    g = x + 1
        #    dy = dy_min*2**(gen_max-g)
        #    y = (i - 2**x - (2**x-1)/2)*dy
        #    """Circular coordinate system to reveal more edges"""
        #    if x > 0:
        #        q = np.tan(y/(x+gen_max))
        #        xcirc = x/np.sqrt(1+q**2)
        #        ycirc = xcirc*q
        #    else:
        #        xcirc, ycirc = 0,0
        #    pos[mapping[i]] = (xcirc,ycirc)


"""
Install python-igraph and cairo:
...
/Users/dghicks/.virtualenvs/pyenv3/lib/python3.5/site-packages/igraph/drawing/__init__.py
